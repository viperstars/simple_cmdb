...
def create(**kwargs):
    ...
    check_params(ServerType, kwargs)
...
def get(**kwargs):
    ...
    check_output(output, ServerType)

    order_by = ("desc", "asc")
    order_list = order.split()

    if not hasattr(ServerType, order_list[0]):
    ...
    order_obj = getattr(getattr(ServerType, order_list[0]), order_list[1])


    if where:
        servertype_list = db.session.query(ServerType).filter_by(**where).order_by(order_obj()).limit(limit).all()
    else:
        servertype_list = db.session.query(ServerType).order_by(order_obj()).limit(limit).all()
    ...
    if output:
        return [output_func(x, output) for x in servertype_list]
    else:
        return [general_output(x) for x in servertype_list]
...
def update(**kwargs):
    ...
    for x, y in data.iteritems():
        if not (hasattr(ServerType, x) and y):
            raise Exception("data wrong")
    servertype = db.session.query(ServerType).filter_by(**where).update(data)
...
def delete(**kwargs):
    ...
    if not("id" in where and where.get("id", None)):
        raise Exception("id not included")
    servertype = db.session.query(ServerType).filter_by(**where).all()

    if servertype:
        try:
            db.session.delete(servertype[0])
            db.session.commit()
        except Exception as e:
            print e
    else:
    ...
    id = servertype[0].id
    servertype = db.session.query(ServerType).filter_by(id=id).all()
    if servertype:
        return id
